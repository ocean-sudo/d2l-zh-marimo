...
def _(d2l, torch):
    ...
    return devices, run, x_gpu1, x_gpu2
...
@app.cell
def _(d2l, devices, run, torch, x_gpu1, x_gpu2):
    run(x_gpu1)
    run(x_gpu2)  # 预热设备
...
@app.cell
def _(d2l, run, torch, x_gpu1, x_gpu2):
...
@app.cell
def _(d2l, run, torch, x_gpu1):
    def copy_to_cpu(x, non_blocking=False):
        return [_y.to('cpu', non_blocking=non_blocking) for _y in x]
    with d2l.Benchmark('在GPU1上运行'):
        _y = run(x_gpu1)
        torch.cuda.synchronize()
    with d2l.Benchmark('复制到CPU'):
        _y_cpu = copy_to_cpu(_y)
        torch.cuda.synchronize()
    return (copy_to_cpu,)
...
@app.cell
def _(copy_to_cpu, d2l, run, torch, x_gpu1):
    with d2l.Benchmark('在GPU1上运行并复制到CPU'):
        _y = run(x_gpu1)
        _y_cpu = copy_to_cpu(_y, True)
        torch.cuda.synchronize()
    return
