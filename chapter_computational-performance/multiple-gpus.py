...
def allreduce(data):
    for i in range(1, len(_data)):
        _data[0][:] += _data[i].to(_data[0].device)
    for i in range(1, len(_data)):
        _data[i][:] = _data[0].to(_data[i].device)
...
@app.cell
def _(d2l, torch):
    _data = [torch.ones((1, 2), device=d2l.try_gpu(i)) * (i + 1) for i in range(2)]
    print('allreduce之前：\n', _data[0], '\n', _data[1])
    allreduce(_data)
    print('allreduce之后：\n', _data[0], '\n', _data[1])
    return
...
@app.cell
def _(nn, torch):
    _data = torch.arange(20).reshape(4, 5)
    devices = [torch.device('cuda:0'), torch.device('cuda:1')]
    split = nn.parallel.scatter(_data, devices)
    print('input :', _data)
...
@app.cell
def _(d2l, lenet, loss, split_batch, torch):
...
@app.cell
def _(d2l, lenet, params, torch, train_batch):
