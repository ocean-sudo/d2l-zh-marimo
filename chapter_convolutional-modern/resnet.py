...
@app.cell
def _(Residual, X):
    _blk = Residual(3, 6, use_1x1conv=True, strides=2)
    _blk(X).shape
    return
    ...
    def resnet_block(input_channels, num_channels, num_residuals, first_block=False):
        ...
        for i in range(num_residuals):
            if i == 0 and (not first_block):
                _blk.append(Residual(input_channels, num_channels, use_1x1conv=True, strides=2))
            else:
                _blk.append(Residual(num_channels, num_channels))
        return _blk
...
@app.cell
def _(net, torch):
    X_1 = torch.rand(size=(1, 1, 224, 224))
    for layer in net:
        X_1 = layer(X_1)
