...
def _(T, torch, x):
    ...
    for _i in range(tau):
        features[:, _i] = x[_i:T - tau + _i]
    labels = x[tau:].reshape((-1, 1))
    return features, labels, tau


@app.cell
def _(d2l, features, labels):
    batch_size, n_train = 16, 600
    # 只有前n_train个样本用于训练
    train_iter = d2l.load_array((features[:n_train], labels[:n_train]),
                                batch_size, is_train=True)
    return n_train, train_iter
...
@app.cell
def _(d2l, get_net, loss, torch, train_iter):
...
@app.cell
def _(d2l, features, net, tau, time, x):
...
@app.cell
def _(T, n_train, net, tau, torch, x):
    multistep_preds = torch.zeros(T)
    multistep_preds[:n_train + tau] = x[:n_train + tau]
    for _i in range(n_train + tau, T):
        multistep_preds[_i] = net(multistep_preds[_i - tau:_i].reshape((1, -1)))
    return (multistep_preds,)


@app.cell
def _(d2l, multistep_preds, n_train, onestep_preds, tau, time, x):
...
@app.cell
def _(T, max_steps, net, tau, torch, x):
    features_1 = torch.zeros((T - tau - max_steps + 1, tau + max_steps))
    for _i in range(tau):
        features_1[:, _i] = x[_i:_i + T - tau - max_steps + 1]
    for _i in range(tau, tau + max_steps):
        features_1[:, _i] = net(features_1[:, _i - tau:_i]).reshape(-1)
    return (features_1,)


@app.cell
def _(T, d2l, features_1, max_steps, tau, time):
    steps = (1, 4, 16, 64)
    d2l.plot([time[tau + _i - 1:T - max_steps + _i] for _i in steps], [features_1[:, tau + _i - 1].detach().numpy() for _i in steps], 'time', 'x', legend=[f'{_i}-step preds' for _i in steps], xlim=[5, 1000], figsize=(6, 3))
    return
