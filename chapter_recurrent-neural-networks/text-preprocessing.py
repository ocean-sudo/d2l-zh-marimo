...
def _(d2l, re):
    ...
    print(lines[0])
    print(lines[10])
    return lines, read_time_machine
...
@app.cell
def _(Vocab, read_time_machine, tokenize):
    def load_corpus_time_machine(max_tokens=-1):  #@save
        """返回时光机器数据集的词元索引列表和词表"""
        lines = read_time_machine()
        tokens = tokenize(lines, 'char')
        vocab = Vocab(tokens)
        corpus = [vocab[token] for line in tokens for token in line]  # 因为时光机器数据集中的每个文本行不一定是一个句子或一个段落，
        if max_tokens > 0:  # 所以将所有文本行展平到一个列表中
            corpus = corpus[:max_tokens]
        return (corpus, vocab)
    corpus, vocab_1 = load_corpus_time_machine()
    (len(corpus), len(vocab_1))
    return
