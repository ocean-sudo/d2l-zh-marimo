...
@app.cell
def _(bigram_vocab, d2l, freqs, trigram_vocab):
...
def _(random, torch):
    def seq_data_iter_random(corpus, batch_size, num_steps):  #@save
        ...
        initial_indices = list(range(0, num_subseqs * num_steps, num_steps))  # 减去1，是因为我们需要考虑标签
        random.shuffle(initial_indices)
      # 长度为num_steps的子序列的起始索引
        def data(pos):
            return corpus[pos:pos + num_steps]  # 在随机抽样的迭代过程中，
        num_batches = num_subseqs // batch_size  # 来自两个相邻的、随机的、小批量中的子序列不一定在原始序列上相邻
        for i in range(0, batch_size * num_batches, batch_size):
            initial_indices_per_batch = initial_indices[i:i + batch_size]
            _X = [data(j) for j in initial_indices_per_batch]
            _Y = [data(j + 1) for j in initial_indices_per_batch]  # 返回从pos位置开始的长度为num_steps的序列
            yield (torch.tensor(_X), torch.tensor(_Y))  # 在这里，initial_indices包含子序列的随机起始索引
...
@app.cell
def _(seq_data_iter_random):
    my_seq = list(range(35))
    for _X, _Y in seq_data_iter_random(my_seq, batch_size=2, num_steps=5):
        print('X: ', _X, '\nY:', _Y)
    return (my_seq,)
...
@app.cell
def _(my_seq, seq_data_iter_sequential):
    for _X, _Y in seq_data_iter_sequential(my_seq, batch_size=2, num_steps=5):
        print('X: ', _X, '\nY:', _Y)
    return
