...
def _():
    ...
    d2l.use_svg_display()
    return d2l, data, torch, torchvision, transforms
...
@app.cell
def _(data, mnist_train, show_images):
    _X, _y = next(iter(data.DataLoader(mnist_train, batch_size=18)))
    show_images(_X.reshape(18, 28, 28), 2, 9, titles=get_fashion_mnist_labels(_y))
    return
...
def _(data, mnist_train):
    ...
    return get_dataloader_workers, train_iter
...
@app.cell
def _(data, get_dataloader_workers, torchvision, transforms):
    ...
    return (load_data_fashion_mnist,)
...
@app.cell
def _(load_data_fashion_mnist):
    train_iter_1, test_iter = load_data_fashion_mnist(32, resize=64)
    for _X, _y in train_iter_1:
        print(_X.shape, _X.dtype, _y.shape, _y.dtype)
        break
    return
