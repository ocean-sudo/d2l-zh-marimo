...
def _():
    ...
    return collections, d2l, math, nn, torch
...
def _(d2l, nn):
    ...
    class Seq2SeqEncoder(d2l.Encoder):
        ...
        def forward(self, X, *args):
            X = self.embedding(X)
            X = X.permute(1, 0, 2)
            _output, state = self.rnn(X)  # 输出'X'的形状：(batch_size,num_steps,embed_size)
            return (_output, state)  # 在循环神经网络模型中，第一个轴对应于时间步  # 如果未提及状态，则默认为0  # output的形状:(num_steps,batch_size,num_hiddens)  # state的形状:(num_layers,batch_size,num_hiddens)
...
@app.cell
def _(Seq2SeqEncoder, torch):
    encoder = Seq2SeqEncoder(vocab_size=10, embed_size=8, num_hiddens=16, num_layers=2)
    encoder.eval()
    X = torch.zeros((4, 7), dtype=torch.long)
    _output, state = encoder(X)
    _output.shape
    return X, encoder, state
...
def _(d2l, nn, torch):
    class Seq2SeqDecoder(d2l.Decoder):
        ...
        def forward(self, X, state):
            ...
            context = state[-1].repeat(X.shape[0], 1, 1)  # 输出'X'的形状：(batch_size,num_steps,embed_size)
            X_and_context = torch.cat((X, context), 2)
            _output, state = self.rnn(X_and_context, state)  # 广播context，使其具有与X相同的num_steps
            _output = self.dense(_output).permute(1, 0, 2)
            return (_output, state)  # output的形状:(batch_size,num_steps,vocab_size)  # state的形状:(num_layers,batch_size,num_hiddens)
...
@app.cell
def _(Seq2SeqDecoder, X, encoder):
    _decoder = Seq2SeqDecoder(vocab_size=10, embed_size=8, num_hiddens=16, num_layers=2)
    _decoder.eval()
    state_1 = _decoder.init_state(encoder(X))
    _output, state_1 = _decoder(X, state_1)
    (_output.shape, state_1.shape)
    return
...
def _(torch):
    ...
    sequence_mask(X_1, torch.tensor([1, 2]))
...
@app.cell
def _(sequence_mask, torch):
    X_2 = torch.ones(2, 3, 4)
    sequence_mask(X_2, torch.tensor([1, 2]), value=-1)
    return
...
@app.cell
def _(MaskedSoftmaxCELoss, torch):
...
@app.cell
def _(MaskedSoftmaxCELoss, d2l, nn, torch):
    #@save
...
@app.cell
def _(Seq2SeqDecoder, Seq2SeqEncoder, d2l, train_seq2seq):
    embed_size, num_hiddens, num_layers, dropout = (32, 32, 2, 0.1)
    batch_size, num_steps = (64, 10)
    lr, num_epochs, device = (0.005, 300, d2l.try_gpu())
    train_iter, src_vocab, tgt_vocab = d2l.load_data_nmt(batch_size, num_steps)
    encoder_1 = Seq2SeqEncoder(len(src_vocab), embed_size, num_hiddens, num_layers, dropout)
    _decoder = Seq2SeqDecoder(len(tgt_vocab), embed_size, num_hiddens, num_layers, dropout)
    net = d2l.EncoderDecoder(encoder_1, _decoder)
    train_seq2seq(net, train_iter, lr, num_epochs, tgt_vocab, device)
    return device, net, num_steps, src_vocab, tgt_vocab
        ...
        for n in range(1, k + 1):
            ...
            for i in range(len_pred - n + 1):
                if label_subs[' '.join(pred_tokens[i:i + n])] > 0:
                    ...
                    label_subs[' '.join(pred_tokens[i:i + n])] = label_subs[' '.join(pred_tokens[i:i + n])] - 1
...
@app.cell
def _(bleu, device, net, num_steps, predict_seq2seq, src_vocab, tgt_vocab):
