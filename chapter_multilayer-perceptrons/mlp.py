...
def _(d2l, torch, x):
    y_1 = torch.sigmoid(x)
    d2l.plot(x.detach(), y_1.detach(), 'x', 'sigmoid(x)', figsize=(5, 2.5))
    return (y_1,)
...
@app.cell
def _(d2l, torch, x, y_1):
    # 清除以前的梯度
    x.grad.data.zero_()
    y_1.backward(torch.ones_like(x), retain_graph=True)
...
def _(d2l, torch, x):
    y_2 = torch.tanh(x)
    d2l.plot(x.detach(), y_2.detach(), 'x', 'tanh(x)', figsize=(5, 2.5))
    return (y_2,)
...
@app.cell
def _(d2l, torch, x, y_2):
    # 清除以前的梯度
    x.grad.data.zero_()
    y_2.backward(torch.ones_like(x), retain_graph=True)
