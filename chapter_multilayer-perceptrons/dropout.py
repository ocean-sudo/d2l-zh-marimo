...
def _():
    ...
    return d2l, dropout_layer, nn, torch
...
def _():
    ...
    return num_hiddens1, num_hiddens2, num_inputs, num_outputs
...
@app.cell
def _(dropout_layer, nn, num_hiddens1, num_hiddens2, num_inputs, num_outputs):
...
@app.cell
def _(d2l, net, nn, torch):
    num_epochs, lr, batch_size = (10, 0.5, 256)
    loss = nn.CrossEntropyLoss(reduction='none')
    train_iter, test_iter = d2l.load_data_fashion_mnist(batch_size)
    _trainer = torch.optim.SGD(net.parameters(), lr=lr)
    d2l.train_ch3(net, train_iter, test_iter, loss, num_epochs, _trainer)
    return loss, lr, num_epochs, test_iter, train_iter
...
@app.cell
def _(dropout1, dropout2, nn):
    net_1 = nn.Sequential(nn.Flatten(), nn.Linear(784, 256), nn.ReLU(), nn.Dropout(dropout1), nn.Linear(256, 256), nn.ReLU(), nn.Dropout(dropout2), nn.Linear(256, 10))

    def init_weights(m):
        if type(m) == nn.Linear:  # 在第一个全连接层之后添加一个dropout层
            nn.init.normal_(m.weight, std=0.01)
    net_1.apply(init_weights)  # 在第二个全连接层之后添加一个dropout层
    return (net_1,)
...
@app.cell
def _(d2l, loss, lr, net_1, num_epochs, test_iter, torch, train_iter):
    _trainer = torch.optim.SGD(net_1.parameters(), lr=lr)
    d2l.train_ch3(net_1, train_iter, test_iter, loss, num_epochs, _trainer)
    return
