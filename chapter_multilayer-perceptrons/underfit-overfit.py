...
def _(math, np):
    ...
    _true_w = np.zeros(max_degree)
    _true_w[0:4] = np.array([5, 1.2, -3.4, 5.6])
    features = np.random.normal(size=(n_train + n_test, 1))
    np.random.shuffle(features)
    poly_features = np.power(features, np.arange(max_degree).reshape(1, -1))
    for i in range(max_degree):
        poly_features[:, i] = poly_features[:, i] / math.gamma(i + 1)
    labels = np.dot(poly_features, _true_w)
    labels = labels + np.random.normal(scale=0.1, size=labels.shape)
    return features, labels, n_train, poly_features
...
@app.cell
def _(features, labels, poly_features, torch):
    _true_w, features_1, poly_features_1, labels_1 = [torch.tensor(x, dtype=torch.float32) for x in [_true_w, features, poly_features, labels]]
    return features_1, labels_1, poly_features_1


@app.cell
def _(features_1, labels_1, poly_features_1):
    (features_1[:2], poly_features_1[:2, :], labels_1[:2])
    return
...
@app.cell
def _(d2l, evaluate_loss, nn, torch):
...
@app.cell
def _(labels_1, n_train, poly_features_1, train):
    # 从多项式特征中选择前4个维度，即1,x,x^2/2!,x^3/3!
    train(poly_features_1[:n_train, :4], poly_features_1[n_train:, :4], labels_1[:n_train], labels_1[n_train:])
    return
...
def _(labels_1, n_train, poly_features_1, train):
    # 从多项式特征中选择前2个维度，即1和x
    train(poly_features_1[:n_train, :2], poly_features_1[n_train:, :2], labels_1[:n_train], labels_1[n_train:])
    return
...
def _(labels_1, n_train, poly_features_1, train):
    # 从多项式特征中选取所有维度
    train(poly_features_1[:n_train, :], poly_features_1[n_train:, :], labels_1[:n_train], labels_1[n_train:], num_epochs=1500)
    return
