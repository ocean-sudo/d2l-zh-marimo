...
def _():
    ...
    return DATA_HUB, DATA_URL, hashlib, os, requests, tarfile, zipfile
...
@app.cell
def _(DATA_HUB, hashlib, os, requests):
...
@app.cell
def _(DATA_HUB, download, os, tarfile, zipfile):
...
@app.cell
def _(all_features, pd):
    # “Dummy_na=True”将“na”（缺失值）视为有效的特征值，并为其创建指示符特征
    all_features_1 = pd.get_dummies(all_features, dummy_na=True)
    all_features_1.shape
    return (all_features_1,)
...
@app.cell
def _(all_features_1, torch, train_data):
    n_train = train_data.shape[0]
    train_features = torch.tensor(all_features_1[:n_train].values, dtype=torch.float32)
    test_features = torch.tensor(all_features_1[n_train:].values, dtype=torch.float32)
...
@app.cell
def _(d2l, get_k_fold_data, get_net, train):
...
@app.cell
def _(k_fold, train_features, train_labels):
    k, num_epochs, lr, weight_decay, batch_size = 5, 100, 5, 0, 64
    train_l, valid_l = k_fold(k, train_features, train_labels, num_epochs, lr,
                              weight_decay, batch_size)
    print(f'{k}-折验证: 平均训练log rmse: {float(train_l):f}, '
          f'平均验证log rmse: {float(valid_l):f}')
    return batch_size, lr, num_epochs, weight_decay
