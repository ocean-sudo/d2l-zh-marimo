    ...
    def get_max_freq_pair(token_freqs):
        ...
        for token, freq in token_freqs.items():
            ...
            for i in range(len(symbols) - 1):
                pairs[symbols[i], symbols[i + 1]] = pairs[symbols[i], symbols[i + 1]] + freq  # “pairs”的键是两个连续符号的元组
        return max(pairs, key=pairs.get)  # 具有最大值的“pairs”键

    return (get_max_freq_pair,)
...
@app.cell
def _(get_max_freq_pair, symbols, token_freqs):
    num_merges = 10
    for i in range(num_merges):
        max_freq_pair = get_max_freq_pair(token_freqs)
        token_freqs_1 = merge_symbols(max_freq_pair, token_freqs, symbols)
        print(f'合并# {i + 1}:', max_freq_pair)
    return (token_freqs_1,)
...
def segment_BPE(tokens, symbols):
    ...
    for token in tokens:
        ...
        cur_output = []
        while start < len(token) and start < end:  # 具有符号中可能最长子字的词元段
