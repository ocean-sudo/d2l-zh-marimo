...
def _():
    ...
    return d2l, math, os, random, torch
...
@app.cell
def _(d2l, math, random, sentences, vocab):
    #@save
    ...
    return counter, subsample, subsampled
...
@app.cell
def _(d2l, sentences, subsampled):
...
def _(random):
    ...
    def get_centers_and_contexts(corpus, max_window_size):
        ...
        centers, contexts = ([], [])
        for line in corpus:
            if len(line) < 2:  # 要形成“中心词-上下文词”对，每个句子至少需要有2个词
                continue
            centers = centers + line
            for i in range(len(line)):
                window_size = random.randint(1, max_window_size)  # 上下文窗口中间i
                indices = list(range(max(0, i - window_size), min(len(line), i + 1 + window_size)))
                indices.remove(i)
                contexts.append([line[idx] for idx in indices])
        return (centers, contexts)  # 从上下文词中排除中心词

    return (get_centers_and_contexts,)
...
@app.cell
def _(get_centers_and_contexts):
...
@app.cell
def _(corpus, get_centers_and_contexts):
...
def _(random):
    ...
    class RandomGenerator:
        ...
        def __init__(self, sampling_weights):  # Exclude
        ...
        def draw(self):
            if self.i == len(self.candidates):
                self.candidates = random.choices(self.population, self.sampling_weights, k=10000)  # 缓存k个随机采样结果
                self.i = 0
            self.i = self.i + 1
...
@app.cell
def _(RandomGenerator, all_contexts, counter, vocab):
    #@save
...
def _(torch):
    def batchify(data):
        """返回带有负采样的跳元模型的小批量样本"""
        max_len = max((len(c) + len(n) for _, c, n in _data))
        centers, contexts_negatives, masks, labels = ([], [], [], [])
        for center, context, negative in _data:
            cur_len = len(context) + len(negative)
            centers = centers + [center]
            contexts_negatives = contexts_negatives + [context + negative + [0] * (max_len - cur_len)]
            masks = masks + [[1] * cur_len + [0] * (max_len - cur_len)]
...
def _(batchify):
    x_1 = (1, [2, 2], [3, 3, 3, 3])
    x_2 = (1, [2, 2, 2], [3, 3])
    _batch = batchify((x_1, x_2))
    names = ['centers', 'contexts_negatives', 'masks', 'labels']
    for _name, _data in zip(names, _batch):
        print(_name, '=', _data)
    return (names,)
...
@app.cell
def _(load_data_ptb, names):
    data_iter, vocab_1 = load_data_ptb(512, 5, 5)
    for _batch in data_iter:
        for _name, _data in zip(names, _batch):
            print(_name, 'shape:', _data.shape)
        break
    return
