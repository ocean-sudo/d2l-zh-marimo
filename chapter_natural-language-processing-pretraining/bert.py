...
def _(BERTEncoder):
    ...
    encoder = BERTEncoder(vocab_size, num_hiddens, norm_shape, ffn_num_input,
                          ffn_num_hiddens, num_heads, num_layers, dropout)
    return encoder, num_hiddens, vocab_size
...
@app.cell
def _(encoder, torch, vocab_size):
...
@app.cell
def _(MaskLM, encoded_X, num_hiddens, torch, vocab_size):
...
@app.cell
def _(mlm_Y_hat, nn, torch, vocab_size):
...
@app.cell
def _(NextSentencePred, encoded_X, torch):
    encoded_X_1 = torch.flatten(encoded_X, start_dim=1)
    # NSP的输入形状:(batchsize，num_hiddens)
    nsp = NextSentencePred(encoded_X_1.shape[-1])
    nsp_Y_hat = nsp(encoded_X_1)
    nsp_Y_hat.shape
    return (nsp_Y_hat,)
...
@app.cell
def _(BERTEncoder, MaskLM, NextSentencePred, nn):
    #@save
