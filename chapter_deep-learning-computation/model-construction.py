...
def _():
    ...
    _net = nn.Sequential(nn.Linear(20, 256), nn.ReLU(), nn.Linear(256, 10))
    X = torch.rand(2, 20)
    _net(X)
    return F, X, nn, torch
...
@app.cell
def _(MySequential, X, nn):
    _net = MySequential(nn.Linear(20, 256), nn.ReLU(), nn.Linear(256, 10))
    _net(X)
    return
