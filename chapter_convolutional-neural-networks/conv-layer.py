...
def _(X_1, Y, nn):
    ...
    X_2 = X_1.reshape((1, 1, 6, 8))
    # 这个二维卷积层使用四维输入和输出格式（批量大小、通道、高度、宽度），
    # 其中批量大小和通道数都为1
    Y_1 = Y.reshape((1, 1, 6, 7))
    lr = 0.03
    for i in range(10):  # 学习率
        Y_hat = conv2d(X_2)
        l = (Y_hat - Y_1) ** 2
        conv2d.zero_grad()
        l.sum().backward()
        ...
        if (i + 1) % 2 == 0:
            print(f'epoch {i + 1}, loss {l.sum():.3f}')  # 迭代卷积核
    return (conv2d,)
