...
@app.cell
def _(K_1, X, corr2d_multi_in_out):
    corr2d_multi_in_out(X, K_1)
    return
...
def _(torch):
    ...
    return (corr2d_multi_in_out_1x1,)
...
@app.cell
def _(torch):
    X_1 = torch.normal(0, 1, (3, 3, 3))
    K_2 = torch.normal(0, 1, (2, 3, 1, 1))
    return K_2, X_1


@app.cell
def _(K_2, X_1, corr2d_multi_in_out, corr2d_multi_in_out_1x1, torch):
    Y1 = corr2d_multi_in_out_1x1(X_1, K_2)
    Y2 = corr2d_multi_in_out(X_1, K_2)
    assert float(torch.abs(Y1 - Y2).sum()) < 1e-06
    return
