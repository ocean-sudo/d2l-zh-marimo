...
def _():
    ...
    return d2l, nn, test_iter, torch, train_iter, vocab
...
@app.cell
def _(corr1d, torch):
    _X, _K = (torch.tensor([0, 1, 2, 3, 4, 5, 6]), torch.tensor([1, 2]))
    corr1d(_X, _K)
    return
...
def _(corr1d, torch):
    def corr1d_multi_in(X, K):
        return sum((corr1d(x, k) for x, k in zip(_X, _K)))  # 首先，遍历'X'和'K'的第0维（通道维）。然后，把它们加在一起
    _X = torch.tensor([[0, 1, 2, 3, 4, 5, 6], [1, 2, 3, 4, 5, 6, 7], [2, 3, 4, 5, 6, 7, 8]])
    _K = torch.tensor([[1, 2], [3, 4], [-1, -3]])
    corr1d_multi_in(_X, _K)
    return
...
@app.cell
def _(d2l, devices, net, nn, test_iter, torch, train_iter):
