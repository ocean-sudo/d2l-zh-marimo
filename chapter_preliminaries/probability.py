...
def _(fair_probs, multinomial):
    # 将结果存储为32位浮点数以进行除法
    _counts = multinomial.Multinomial(1000, fair_probs).sample()
    _counts / 1000  # 相对频率作为估计值
    return
...
@app.cell
def _(d2l, fair_probs, multinomial):
    _counts = multinomial.Multinomial(10, fair_probs).sample((500,))
    cum_counts = _counts.cumsum(dim=0)
