...
def _(n_train, torch, x_train):
    ...
    n_test
    return n_test, x_test, y_train, y_truth
...
@app.cell
def _(d2l, x_test, x_train, y_train, y_truth):
    def plot_kernel_reg(y_hat):
        d2l.plot(x_test, [y_truth, _y_hat], 'x', 'y', legend=['Truth', 'Pred'], xlim=[0, 5], ylim=[-1, 5])
        d2l.plt.plot(x_train, y_train, 'o', alpha=0.5)

    return (plot_kernel_reg,)
...
@app.cell
def _(n_test, plot_kernel_reg, torch, y_train):
    _y_hat = torch.repeat_interleave(y_train.mean(), n_test)
    plot_kernel_reg(_y_hat)
    return
...
@app.cell
def _(n_train, nn, plot_kernel_reg, torch, x_test, x_train, y_train):
    ...
    _y_hat = torch.matmul(attention_weights, y_train)
    plot_kernel_reg(_y_hat)
...
@app.cell
def _(n_train, torch, x_train, y_train):
    ...
    values_1 = Y_tile[(1 - torch.eye(n_train)).type(torch.bool)].reshape((n_train, -1))
    return keys, values_1
...
@app.cell
def _(NWKernelRegression, d2l, keys, nn, torch, values_1, x_train, y_train):
    ...
    for epoch in range(5):
        trainer.zero_grad()
        l = loss(net(x_train, keys, values_1), y_train)
        l.sum().backward()
        trainer.step()
...
@app.cell
def _(n_test, net, plot_kernel_reg, x_test, x_train, y_train):
    keys_1 = x_train.repeat((n_test, 1))
    values_2 = y_train.repeat((n_test, 1))
    _y_hat = net(x_test, keys_1, values_2).unsqueeze(1).detach()
    plot_kernel_reg(_y_hat)
    return
