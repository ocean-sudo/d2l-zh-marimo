...
def _(d2l):
    ...
    attention.eval()
    return attention, num_hiddens


@app.cell
def _(attention, num_hiddens, torch):
    batch_size, num_queries, valid_lens = (2, 4, torch.tensor([3, 2]))
    _X = torch.ones((batch_size, num_queries, num_hiddens))
    attention(_X, _X, _X, valid_lens).shape
    return
...
def _(nn, torch):
    ...
    class PositionalEncoding(nn.Module):
        ...
        def __init__(self, num_hiddens, dropout, max_len=1000):
            ...
            _X = torch.arange(max_len, dtype=torch.float32).reshape(-1, 1) / torch.pow(10000, torch.arange(0, num_hiddens, 2, dtype=torch.float32) / num_hiddens)
            self.P[:, :, 0::2] = torch.sin(_X)
            self.P[:, :, 1::2] = torch.cos(_X)

        def forward(self, X):
            _X = _X + self.P[:, :_X.shape[1], :].to(_X.device)
...
@app.cell
def _(PositionalEncoding, d2l, torch):
    encoding_dim, num_steps = (32, 60)
    pos_encoding = PositionalEncoding(encoding_dim, 0)
    pos_encoding.eval()
    _X = pos_encoding(torch.zeros((1, num_steps, encoding_dim)))
    P = pos_encoding.P[:, :_X.shape[1], :]
...
@app.cell
def _(P, d2l):
    P_1 = P[0, :, :].unsqueeze(0).unsqueeze(0)
    d2l.show_heatmaps(P_1, xlabel='Column (encoding dimension)', ylabel='Row (position)', figsize=(3.5, 4), cmap='Blues')
    return
