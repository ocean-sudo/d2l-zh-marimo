...
def _(nn):
    ...
    class PositionWiseFFN(nn.Module):
        ...
        def forward(self, X):
            return self.dense2(self.relu(self.dense1(_X)))
...
def _(nn, torch):
    ln = nn.LayerNorm(2)
    bn = nn.BatchNorm1d(2)
    _X = torch.tensor([[1, 2], [2, 3]], dtype=torch.float32)
    # 在训练模式下计算X的均值和方差
    print('layer norm:', ln(_X), '\nbatch norm:', bn(_X))
    return
...
def _(nn):
    ...
    class AddNorm(nn.Module):
        ...
        def forward(self, X, Y):
            return self.ln(self.dropout(Y) + _X)
...
@app.cell
def _(AddNorm, PositionWiseFFN, d2l, nn):
    #@save
    class EncoderBlock(nn.Module):
        ...
        def forward(self, X, valid_lens):
            Y = self.addnorm1(_X, self.attention(_X, _X, _X, valid_lens))
...
@app.cell
def _(EncoderBlock, torch):
    _X = torch.ones((2, 100, 24))
    valid_lens = torch.tensor([3, 2])
    encoder_blk = EncoderBlock(24, 24, 24, 24, [100, 24], 24, 48, 8, 0.5)
    encoder_blk.eval()
    encoder_blk(_X, valid_lens).shape
    return encoder_blk, valid_lens
...
@app.cell
def _(EncoderBlock, d2l, math, nn):
    #@save
    class TransformerEncoder(d2l.Encoder):
        ...
        def forward(self, X, valid_lens, *args):
            _X = self.pos_encoding(self.embedding(_X) * math.sqrt(self.num_hiddens))
            self.attention_weights = [None] * len(self.blks)
            for i, blk in enumerate(self.blks):
                _X = blk(_X, valid_lens)
                self.attention_weights[i] = blk.attention.attention.attention_weights  # 因为位置编码值在-1和1之间，
            return _X  # 因此嵌入值乘以嵌入维度的平方根进行缩放，  # 然后再与位置编码相加。
...
@app.cell
def _(TransformerEncoder, torch, valid_lens):
    _encoder = TransformerEncoder(200, 24, 24, 24, 24, [100, 24], 24, 48, 8, 2, 0.5)
    _encoder.eval()
    _encoder(torch.ones((2, 100), dtype=torch.long), valid_lens).shape
    return
...
@app.cell
def _(AddNorm, PositionWiseFFN, d2l, nn, torch):
    class DecoderBlock(nn.Module):
        ...
        def forward(self, X, state):
            ...
            if state[2][self.i] is None:
                ...
                key_values = torch.cat((state[2][self.i], _X), axis=1)
            state[2][self.i] = key_values  # 训练阶段，输出序列的所有词元都在同一时间处理，
            if self.training:  # 因此state[2][self.i]初始化为None。
                batch_size, num_steps, _ = _X.shape  # 预测阶段，输出序列是通过词元一个接着一个解码的，
                dec_valid_lens = torch.arange(1, num_steps + 1, device=_X.device).repeat(batch_size, 1)  # 因此state[2][self.i]包含着直到当前时间步第i个块解码的输出表示
            else:
                dec_valid_lens = None
            X2 = self.attention1(_X, key_values, key_values, dec_valid_lens)
            Y = self.addnorm1(_X, X2)
            Y2 = self.attention2(Y, enc_outputs, enc_outputs, enc_valid_lens)
            Z = self.addnorm2(Y, Y2)
            return (self.addnorm3(Z, self.ffn(Z)), state)  # dec_valid_lens的开头:(batch_size,num_steps),  # 其中每一行是[1,2,...,num_steps]  # 自注意力  # 编码器－解码器注意力。  # enc_outputs的开头:(batch_size,num_steps,num_hiddens)
...
@app.cell
def _(DecoderBlock, encoder_blk, torch, valid_lens):
    decoder_blk = DecoderBlock(24, 24, 24, 24, [100, 24], 24, 48, 8, 0.5, 0)
    decoder_blk.eval()
    _X = torch.ones((2, 100, 24))
    state = [encoder_blk(_X, valid_lens), valid_lens, [None]]
    decoder_blk(_X, state)[0].shape
    return
...
@app.cell
def _(DecoderBlock, d2l, math, nn):
    class TransformerDecoder(d2l.AttentionDecoder):
        ...
        def forward(self, X, state):
            ...
            for i, blk in enumerate(self.blks):
                _X, state = blk(_X, state)
                self._attention_weights[0][i] = blk.attention1.attention.attention_weights
                self._attention_weights[1][i] = blk.attention2.attention.attention_weights
            return (self.dense(_X), state)

        @property  # 解码器自注意力权重
        def attention_weights(self):
            return self._attention_weights  # “编码器－解码器”自注意力权重
...
@app.cell
def _(TransformerDecoder, TransformerEncoder, d2l):
    ...
    _encoder = TransformerEncoder(len(src_vocab), key_size, query_size, value_size, num_hiddens, norm_shape, ffn_num_input, ffn_num_hiddens, num_heads, num_layers, dropout)
    ...
    net = d2l.EncoderDecoder(_encoder, decoder)
    d2l.train_seq2seq(net, train_iter, lr, num_epochs, tgt_vocab, device)
    return device, net, num_heads, num_layers, num_steps, src_vocab, tgt_vocab
...
@app.cell
def _(d2l, device, net, num_steps, src_vocab, tgt_vocab):
    ...
    return dec_attention_weight_seq, translation
...
@app.cell
def _(net, num_heads, num_layers, num_steps, torch):
    enc_attention_weights = torch.cat(net.encoder.attention_weights, 0).reshape((num_layers, num_heads,
        -1, num_steps))
    enc_attention_weights.shape
    return (enc_attention_weights,)
...
@app.cell
def _(d2l, enc_attention_weights):
    d2l.show_heatmaps(
...
@app.cell
def _(dec_attention_weight_seq, num_heads, num_layers, num_steps, pd, torch):
    ...
    return dec_inter_attention_weights, dec_self_attention_weights
...
@app.cell
def _(d2l, dec_self_attention_weights, translation):
...
@app.cell
def _(d2l, dec_inter_attention_weights):
    d2l.show_heatmaps(
