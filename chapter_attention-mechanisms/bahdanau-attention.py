...
@app.cell
def _(AttentionDecoder, d2l, nn, torch):
    ...
    return (Seq2SeqAttentionDecoder,)
...
@app.cell
def _(Seq2SeqAttentionDecoder, d2l, torch):
    _encoder = d2l.Seq2SeqEncoder(vocab_size=10, embed_size=8, num_hiddens=16, num_layers=2)
    _encoder.eval()
    _decoder = Seq2SeqAttentionDecoder(vocab_size=10, embed_size=8, num_hiddens=16, num_layers=2)
    _decoder.eval()
    X = torch.zeros((4, 7), dtype=torch.long)
    state = _decoder.init_state(_encoder(X), None)
    output, state = _decoder(X, state)  # (batch_size,num_steps)
...
@app.cell
def _(Seq2SeqAttentionDecoder, d2l):
    embed_size, num_hiddens, num_layers, dropout = (32, 32, 2, 0.1)
    batch_size, num_steps = (64, 10)
    lr, num_epochs, device = (0.005, 250, d2l.try_gpu())
    train_iter, src_vocab, tgt_vocab = d2l.load_data_nmt(batch_size, num_steps)
    _encoder = d2l.Seq2SeqEncoder(len(src_vocab), embed_size, num_hiddens, num_layers, dropout)
    _decoder = Seq2SeqAttentionDecoder(len(tgt_vocab), embed_size, num_hiddens, num_layers, dropout)
    net = d2l.EncoderDecoder(_encoder, _decoder)
...
def _(d2l, device, net, num_steps, src_vocab, tgt_vocab):
    ...
    return dec_attention_weight_seq, engs


@app.cell
def _(dec_attention_weight_seq, num_steps, torch):
...
@app.cell
def _(attention_weights, d2l, engs):
    # 加上一个包含序列结束词元
    d2l.show_heatmaps(
