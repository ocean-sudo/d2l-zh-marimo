...
def _(masked_softmax, nn, torch):
    ...
    class AdditiveAttention(nn.Module):
        ...
        def forward(self, queries, keys, values, valid_lens):
            _queries, keys = (self.W_q(_queries), self.W_k(keys))  # 在维度扩展后，
            features = _queries.unsqueeze(2) + keys.unsqueeze(1)  # queries的形状：(batch_size，查询的个数，1，num_hidden)
            features = torch.tanh(features)  # key的形状：(batch_size，1，“键－值”对的个数，num_hiddens)
            scores = self.w_v(features).squeeze(-1)  # 使用广播方式进行求和
            self.attention_weights = masked_softmax(scores, valid_lens)
            return torch.bmm(self.dropout(self.attention_weights), values)  # self.w_v仅有一个输出，因此从形状中移除最后那个维度。  # scores的形状：(batch_size，查询的个数，“键-值”对的个数)  # values的形状：(batch_size，“键－值”对的个数，值的维度)
...
@app.cell
def _(AdditiveAttention, torch):
    _queries, keys = (torch.normal(0, 1, (2, 1, 20)), torch.ones((2, 10, 2)))
    # values的小批量，两个值矩阵是相同的
    values = torch.arange(40, dtype=torch.float32).reshape(1, 10, 4).repeat(2, 1, 1)
    valid_lens = torch.tensor([2, 6])
    attention = AdditiveAttention(key_size=2, query_size=20, num_hiddens=8, dropout=0.1)
    attention.eval()
    attention(_queries, keys, values, valid_lens)
    return attention, keys, valid_lens, values
...
@app.cell
def _(masked_softmax, math, nn, torch):
    #@save
    class DotProductAttention(nn.Module):
      ...
      # queries的形状：(batch_size，查询的个数，d)
        def forward(self, queries, keys, values, valid_lens=None):  # keys的形状：(batch_size，“键－值”对的个数，d)
            d = _queries.shape[-1]  # values的形状：(batch_size，“键－值”对的个数，值的维度)
            scores = torch.bmm(_queries, keys.transpose(1, 2)) / math.sqrt(d)  # valid_lens的形状:(batch_size，)或者(batch_size，查询的个数)
            self.attention_weights = masked_softmax(scores, valid_lens)
            return torch.bmm(self.dropout(self.attention_weights), values)  # 设置transpose_b=True为了交换keys的最后两个维度
...
@app.cell
def _(DotProductAttention, keys, torch, valid_lens, values):
    _queries = torch.normal(0, 1, (2, 1, 2))
    attention_1 = DotProductAttention(dropout=0.5)
    attention_1.eval()
    attention_1(_queries, keys, values, valid_lens)
    return (attention_1,)
...
@app.cell
def _(attention_1, d2l):
    d2l.show_heatmaps(attention_1.attention_weights.reshape((1, 1, 2, 10)), xlabel='Keys', ylabel='Queries')
    return
