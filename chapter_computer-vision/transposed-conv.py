...
def _(mo):
    ...
    ![卷积核为 $2\times 2$ 的转置卷积。阴影部分是中间张量的一部分，也是用于计算的输入和卷积核张量元素。 ](http://d2l.ai/_images/trans_conv.svg)
    ...
    def trans_conv(X, K):
        ...
        for i in range(X.shape[0]):
            for j in range(X.shape[1]):
                Y[i:i + h, j:j + w] = Y[i:i + h, j:j + w] + X[i, j] * K
        return Y
...
@app.cell
def _(K, X, nn):
    X_1, K_1 = (X.reshape(1, 1, 2, 2), K.reshape(1, 1, 2, 2))
    _tconv = nn.ConvTranspose2d(1, 1, kernel_size=2, bias=False)
    _tconv.weight.data = K_1
    _tconv(X_1)
    return K_1, X_1
...
@app.cell
def _(K_1, X_1, nn):
    _tconv = nn.ConvTranspose2d(1, 1, kernel_size=2, padding=1, bias=False)
    _tconv.weight.data = K_1
    _tconv(X_1)
    return
...
def _(mo):
    ...
    ![卷积核为$2\times 2$，步幅为2的转置卷积。阴影部分是中间张量的一部分，也是用于计算的输入和卷积核张量元素。](http://d2l.ai/_images/trans_conv_stride2.svg)
...
@app.cell
def _(nn, torch):
    X_2 = torch.rand(size=(1, 10, 16, 16))
    conv = nn.Conv2d(10, 20, kernel_size=5, padding=2, stride=3)
    _tconv = nn.ConvTranspose2d(20, 10, kernel_size=5, padding=2, stride=3)
    _tconv(conv(X_2)).shape == X_2.shape
    return
...
@app.cell
def _(d2l, torch):
    X_3 = torch.arange(9.0).reshape(3, 3)
    K_2 = torch.tensor([[1.0, 2.0], [3.0, 4.0]])
    Y = d2l.corr2d(X_3, K_2)
    Y
    return K_2, X_3, Y
...
@app.cell
def _(W, X_3, Y, torch):
    Y == torch.matmul(W, X_3.reshape(-1)).reshape(2, 2)
    return
...
@app.cell
def _(K_2, W, Y, torch, trans_conv):
    Z = trans_conv(Y, K_2)
