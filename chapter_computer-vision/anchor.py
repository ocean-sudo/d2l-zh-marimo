...
@app.cell
def _(d2l, multibox_prior, torch):
...
@app.cell
def _(boxes, d2l, h, img, show_bboxes, torch, w):
    d2l.set_figsize()
    bbox_scale = torch.tensor((w, h, w, h))
    _fig = d2l.plt.imshow(img)
    show_bboxes(_fig.axes, boxes[250, 250, :, :] * bbox_scale, ['s=0.75, r=1', 's=0.5, r=1', 's=0.25, r=1', 's=0.75, r=2', 's=0.75, r=0.5'])
    return (bbox_scale,)
...
@app.cell
def _(assign_anchor_to_bbox, offset_boxes, torch):
    #@save
...
@app.cell
def _(bbox_scale, d2l, img, show_bboxes, torch):
    ...
    _fig = d2l.plt.imshow(img)
    show_bboxes(_fig.axes, ground_truth[:, 1:] * bbox_scale, ['dog', 'cat'], 'k')
    show_bboxes(_fig.axes, anchors * bbox_scale, ['0', '1', '2', '3', '4'])
    return anchors, ground_truth
...
@app.cell
def _(anchors, ground_truth, multibox_target):
...
@app.cell
def _(nms, offset_inverse, torch):
    #@save
...
@app.cell
def _(torch):
    anchors_1 = torch.tensor([[0.1, 0.08, 0.52, 0.92], [0.08, 0.2, 0.56, 0.95], [0.15, 0.3, 0.62, 0.91], [0.55, 0.2, 0.9, 0.88]])
    offset_preds = torch.tensor([0] * anchors_1.numel())
    cls_probs = torch.tensor([[0] * 4, [0.9, 0.8, 0.7, 0.1], [0.1, 0.2, 0.3, 0.9]])  # 背景的预测概率  # 狗的预测概率  # 猫的预测概率
    return anchors_1, cls_probs, offset_preds
...
@app.cell
def _(anchors_1, bbox_scale, d2l, img, show_bboxes):
    _fig = d2l.plt.imshow(img)
    show_bboxes(_fig.axes, anchors_1 * bbox_scale, ['dog=0.9', 'dog=0.8', 'dog=0.7', 'cat=0.9'])
    return
...
@app.cell
def _(anchors_1, cls_probs, multibox_detection, offset_preds):
    output = multibox_detection(cls_probs.unsqueeze(dim=0), offset_preds.unsqueeze(dim=0), anchors_1.unsqueeze(dim=0), nms_threshold=0.5)
    output
    return (output,)
...
@app.cell
def _(bbox_scale, d2l, img, output, show_bboxes, torch):
    _fig = d2l.plt.imshow(img)
    for i in output[0].detach().numpy():
        if i[0] == -1:
            continue
        label = ('dog=', 'cat=')[int(i[0])] + str(i[1])
        show_bboxes(_fig.axes, [torch.tensor(i[2:]) * bbox_scale], label)
    return
