...
def _():
    ...
    return d2l, os, torch, torchvision
...
def _(os, torchvision, voc_dir):
    ...
    return read_voc_images, train_features, train_labels
...
@app.cell
def _(d2l, train_features, train_labels):
    n = 5
    _imgs = train_features[0:n] + train_labels[0:n]
    _imgs = [img.permute(1, 2, 0) for img in _imgs]
    d2l.show_images(_imgs, 2, n)
    return (n,)
...
def _(VOC_COLORMAP, torch):
    ...
    return voc_colormap2label, voc_label_indices
...
@app.cell
def _(VOC_CLASSES, train_labels, voc_colormap2label, voc_label_indices):
...
def _(torchvision):
    ...
    return (voc_rand_crop,)


@app.cell
def _(d2l, n, train_features, train_labels, voc_rand_crop):
    _imgs = []
    for _ in range(n):
        _imgs += voc_rand_crop(train_features[0], train_labels[0], 200, 300)
    _imgs = [img.permute(1, 2, 0) for img in _imgs]
    d2l.show_images(_imgs[::2] + _imgs[1::2], 2, n)
    return
...
def _(
    ...
    class VOCSegDataset(torch.utils.data.Dataset):
        ...
        def filter(self, imgs):
            return [img for img in _imgs if img.shape[1] >= self.crop_size[0] and img.shape[2] >= self.crop_size[1]]
...
@app.cell
def _(VOCSegDataset, d2l, os, torch):
    #@save
