...
def _():
    ...
    return d2l, nn, os, torch, torchvision
...
def _(nn):
    ...
    def evaluate_loss(data_iter, net, devices):
        ...
        for features, labels in data_iter:
            ...
            outputs = _net(features)
            l = loss(outputs, labels)
            l_sum += l.sum()
            n += labels.numel()
        return (l_sum / n).to('cpu')

    return evaluate_loss, loss
...
@app.cell
def _(d2l, evaluate_loss, loss, nn, torch):
    def train(net, train_iter, valid_iter, num_epochs, lr, wd, devices, lr_period, lr_decay):
        _net = nn.DataParallel(_net, device_ids=devices).to(devices[0])
        trainer = torch.optim.SGD((param for param in _net.parameters() if param.requires_grad), lr=lr, momentum=0.9, weight_decay=wd)  # 只训练小型自定义输出网络
        ...
        for epoch in range(num_epochs):
            ...
            if valid_iter is not None:
                valid_loss = evaluate_loss(valid_iter, _net, devices)
...
def _(d2l, get_net, train, train_iter, valid_iter):
    devices, num_epochs, lr, wd = (d2l.try_all_gpus(), 10, 0.0001, 0.0001)
    lr_period, lr_decay, _net = (2, 0.9, get_net(devices))
...
def _(
    ...
    _net = get_net(devices)
    train(_net, train_valid_iter, None, num_epochs, lr, wd, devices, lr_period, lr_decay)
    preds = []
    for data, label in test_iter:
        output = torch.nn.functional.softmax(_net(data.to(devices[0])), dim=1)
