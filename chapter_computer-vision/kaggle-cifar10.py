...
def _():
    ...
    return collections, d2l, math, nn, os, pd, shutil, torch, torchvision
...
@app.cell
def _(collections, math, os, shutil):
    #@save
    ...
    return copyfile, reorg_train_valid
...
@app.cell
def _(os, read_csv_labels, reorg_test, reorg_train_valid):
    def reorg_cifar10_data(data_dir, valid_ratio):
        labels = read_csv_labels(os.path.join(data_dir, 'trainLabels.csv'))
        reorg_train_valid(data_dir, labels, valid_ratio)
        reorg_test(data_dir)

    return (reorg_cifar10_data,)
...
@app.cell
def _(data_dir, demo, reorg_cifar10_data):
...
@app.cell
def _(data_dir, os, torchvision, transform_test, transform_train):
    ...
    return test_ds, train_ds, train_valid_ds, valid_ds
...
@app.cell
def _(batch_size, test_ds, torch, train_ds, train_valid_ds, valid_ds):
    ...
    test_iter = torch.utils.data.DataLoader(test_ds, batch_size, shuffle=False,
                                            drop_last=False)
    return test_iter, train_iter, train_valid_iter, valid_iter
...
def _(d2l, nn):
    def get_net():
        num_classes = 10
        _net = d2l.resnet18(num_classes, 3)
        return _net
...
@app.cell
def _(d2l, loss, nn, torch):
    def train(net, train_iter, valid_iter, num_epochs, lr, wd, devices, lr_period, lr_decay):
        trainer = torch.optim.SGD(_net.parameters(), lr=lr, momentum=0.9, weight_decay=wd)
        ...
        _net = nn.DataParallel(_net, device_ids=devices).to(devices[0])
        for epoch in range(num_epochs):
            _net.train()
            metric = d2l.Accumulator(3)
            for i, (features, labels) in enumerate(train_iter):
                timer.start()
                l, acc = d2l.train_batch_ch13(_net, features, labels, loss, trainer, devices)
                metric.add(l, acc, labels.shape[0])
                timer.stop()
                if (i + 1) % (num_batches // 5) == 0 or i == num_batches - 1:
                    animator.add(epoch + (i + 1) / num_batches, (metric[0] / metric[2], metric[1] / metric[2], None))
            if valid_iter is not None:
                valid_acc = d2l.evaluate_accuracy_gpu(_net, valid_iter)
...
@app.cell
def _(d2l, get_net, train, train_iter, valid_iter):
    devices, num_epochs, lr, wd = (d2l.try_all_gpus(), 20, 0.0002, 0.0005)
    lr_period, lr_decay, _net = (4, 0.9, get_net())
    train(_net, train_iter, valid_iter, num_epochs, lr, wd, devices, lr_period, lr_decay)
    return devices, lr, lr_decay, lr_period, num_epochs, wd
...
def _(
    ...
    _net, preds = (get_net(), [])
    train(_net, train_valid_iter, None, num_epochs, lr, wd, devices, lr_period, lr_decay)
    for X, _ in test_iter:
        y_hat = _net(X.to(devices[0]))
