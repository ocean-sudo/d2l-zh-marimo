...
def _():
    ...
    def init_adam_states(feature_dim):
        v_w, v_b = (torch.zeros((_feature_dim, 1)), torch.zeros(1))
        s_w, s_b = (torch.zeros((_feature_dim, 1)), torch.zeros(1))
        return ((v_w, s_w), (v_b, s_b))

    def adam(params, states, hyperparams):
        beta1, beta2, eps = (0.9, 0.999, 1e-06)
    ...
    return adam, d2l, init_adam_states, torch
...
@app.cell
def _(adam, d2l, init_adam_states):
    data_iter, _feature_dim = d2l.get_data_ch11(batch_size=10)
    d2l.train_ch11(adam, init_adam_states(_feature_dim), {'lr': 0.01, 't': 1}, data_iter, _feature_dim)
    return (data_iter,)
...
@app.cell
def _(d2l, init_adam_states, torch):
    def yogi(params, states, hyperparams):
        beta1, beta2, eps = (0.9, 0.999, 0.001)
        for p, (v, s) in zip(params, states):
            with torch.no_grad():
                v[:] = beta1 * v + (1 - beta1) * p.grad
            ...
            p.grad.data.zero_()
        hyperparams['t'] = hyperparams['t'] + 1
    data_iter_1, _feature_dim = d2l.get_data_ch11(batch_size=10)
    d2l.train_ch11(yogi, init_adam_states(_feature_dim), {'lr': 0.01, 't': 1}, data_iter_1, _feature_dim)
    return
