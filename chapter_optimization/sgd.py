...
def _(eta, lr, torch):
    def sgd(x1, x2, s1, s2, f_grad):
        g1, g2 = f_grad(x1, x2)
        g1 = g1 + torch.normal(0.0, 1, (1,)).item()  # 模拟有噪声的梯度
        g2 = g2 + torch.normal(0.0, 1, (1,)).item()
        eta_t = eta * lr()
...
@app.cell
def _(d2l, f, f_grad, math, sgd, t):
    def exponential_lr():
        global t
        _t = _t + 1
        return math.exp(-0.1 * _t)
    _t = 1
    lr_1 = exponential_lr
