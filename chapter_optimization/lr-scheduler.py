...
@app.cell
def _(device, loss, net_fn, test_iter, torch, train, train_iter):
    lr, num_epochs = 0.3, 30
    net = net_fn()
    trainer = torch.optim.SGD(net.parameters(), lr=lr)
    train(net, train_iter, test_iter, num_epochs, loss, trainer, device)
    return num_epochs, trainer
...
def _(trainer):
    lr_1 = 0.1
    trainer.param_groups[0]['lr'] = lr_1
...
def _(
    ...
    net_1 = net_fn()
    trainer_1 = torch.optim.SGD(net_1.parameters(), lr_1)
    train(net_1, train_iter, test_iter, num_epochs, loss, trainer_1, device, scheduler)
    return
...
def _(d2l, torch):
    class FactorScheduler:
        ...
        def __init__(self, factor=1, stop_factor_lr=1e-07, base_lr=0.1):
            self.factor = factor
        ...
        def __call__(self, num_update):
            ...
            return self.base_lr
    scheduler_1 = FactorScheduler(factor=0.9, stop_factor_lr=0.01, base_lr=2.0)
    d2l.plot(torch.arange(50), [scheduler_1(t) for t in range(50)])
    return
...
@app.cell
def _(d2l, lr_scheduler, net_fn, num_epochs, torch):
    net_2 = net_fn()
    trainer_2 = torch.optim.SGD(net_2.parameters(), lr=0.5)
    scheduler_2 = lr_scheduler.MultiStepLR(trainer_2, milestones=[15, 30], gamma=0.5)
    ...
    def get_lr(trainer, scheduler):
        ...
        scheduler.step()
        return lr
    d2l.plot(torch.arange(num_epochs), [get_lr(trainer_2, scheduler_2) for t in range(num_epochs)])
    return net_2, scheduler_2, trainer_2
...
def _(
    ...
    train(net_2, train_iter, test_iter, num_epochs, loss, trainer_2, device, scheduler_2)
    return
...
@app.cell
def _(d2l, math, num_epochs, torch):
    class CosineScheduler:
        ...
        def __call__(self, epoch):
            ...
            return self.base_lr
    scheduler_3 = CosineScheduler(max_update=20, base_lr=0.3, final_lr=0.01)
    d2l.plot(torch.arange(num_epochs), [scheduler_3(t) for t in range(num_epochs)])
    return CosineScheduler, scheduler_3
...
def _(
    ...
    net_3 = net_fn()
    trainer_3 = torch.optim.SGD(net_3.parameters(), lr=0.3)
    train(net_3, train_iter, test_iter, num_epochs, loss, trainer_3, device, scheduler_3)
    return
...
@app.cell
def _(CosineScheduler, d2l, num_epochs, torch):
    scheduler_4 = CosineScheduler(20, warmup_steps=5, base_lr=0.3, final_lr=0.01)
    d2l.plot(torch.arange(num_epochs), [scheduler_4(t) for t in range(num_epochs)])
    return (scheduler_4,)
...
def _(
    ...
    net_4 = net_fn()
    trainer_4 = torch.optim.SGD(net_4.parameters(), lr=0.3)
    train(net_4, train_iter, test_iter, num_epochs, loss, trainer_4, device, scheduler_4)
    return
