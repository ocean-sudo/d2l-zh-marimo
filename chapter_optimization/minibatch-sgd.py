...
def _():
    ...
    return A, B, C, d2l, nn, np, timer, torch
...
@app.cell
def _(A, B, C, timer, torch):
    # 逐元素计算A=BC
    timer.start()
    for i in range(256):
        for _j in range(256):
            A[i, _j] = torch.dot(B[i, :], C[:, _j])
    timer.stop()
    return
...
@app.cell
def _(A_1, B, C, timer, torch):
    timer.start()
    for _j in range(0, 256, 64):
        A_1[:, _j:_j + 64] = torch.mm(B, C[:, _j:_j + 64])
    timer.stop()
...
@app.cell
def _(get_data_ch11, train_ch11):
...
@app.cell
def _(d2l, gd_res, mini1_res, mini2_res, sgd_res):
    d2l.set_figsize([6, 3])
...
def _(d2l, nn, torch):
    ...
    def train_concise_ch11(trainer_fn, hyperparams, data_iter, num_epochs=4):
        net = nn.Sequential(nn.Linear(5, 1))  # 初始化模型
        ...
        for _ in range(num_epochs):
            for X, y in data_iter:
                ...
                if n % 200 == 0:
                    ...
                    timer.start()
        print(f'loss: {animator.Y[0][-1]:.3f}, {timer.avg():.3f} sec/epoch')  # MSELoss计算平方误差时不带系数1/2
...
@app.cell
def _(get_data_ch11, torch, train_concise_ch11):
