...
def _(d2l, torch):
    ...
    for _gamma in gammas:
        ...
        d2l.plt.plot(x, (1 - _gamma) * _gamma ** x, label=f'gamma = {_gamma:.2f}')
    d2l.plt.xlabel('time')
    return
...
def _(d2l, math):
    def rmsprop_2d(x1, x2, s1, s2):
        g1, g2, eps = (0.2 * x1, 4 * x2, 1e-06)
        s1 = _gamma * s1 + (1 - _gamma) * g1 ** 2
        s2 = _gamma * s2 + (1 - _gamma) * g2 ** 2
...
def _(torch):
    ...
    return (init_rmsprop_states,)


@app.cell
def _(torch):
    def rmsprop(params, states, hyperparams):
        _gamma, eps = (hyperparams['gamma'], 1e-06)
        for p, s in zip(params, states):
            with torch.no_grad():
                s[:] = _gamma * s + (1 - _gamma) * torch.square(p.grad)
...
@app.cell
def _(d2l, init_rmsprop_states, rmsprop):
