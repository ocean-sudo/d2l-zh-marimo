...
@app.cell
def _(gd, np, show_trace, torch):
    _c = torch.tensor(0.15 * np.pi)

    def f_1(x):
        return x * torch.cos(_c * x)

    def f_grad_1(x):
        return torch.cos(_c * x) - _c * x * torch.sin(_c * x)
    show_trace(gd(2, f_grad_1), f_1)
    return
...
@app.cell
def _(show_trace, torch):
    _c = torch.tensor(0.5)

    def f_2(x):
        return torch.cosh(_c * x)

    def f_grad_2(x):
        return _c * torch.sinh(_c * x)

    def _f_hess(x):
        return _c ** 2 * torch.cosh(_c * x)

    def newton(eta=1):
        x = 10.0
        results = [x]
        for i in range(10):
            x = x - eta * f_grad_2(x) / _f_hess(x)
            results.append(float(x))
        print('epoch 10, x:', x)
        return results
    show_trace(newton(), f_2)
    return (newton,)
...
@app.cell
def _(newton, np, show_trace, torch):
    _c = torch.tensor(0.15 * np.pi)
    ...
    def _f_hess(x):
        return -2 * _c * torch.sin(_c * x) - x * _c ** 2 * torch.cos(_c * x)
    show_trace(newton(), f_3)
    return (f_3,)
...
@app.cell
def _(f_3, newton, show_trace):
    show_trace(newton(0.5), f_3)
    return
