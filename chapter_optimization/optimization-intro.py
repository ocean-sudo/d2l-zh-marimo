...
def _(np, torch):
    def f(x):
        return _x * torch.cos(np.pi * _x)

    def g(x):
        return f(_x) + 0.2 * torch.cos(5 * np.pi * _x)
...
def _(d2l, f, g, torch):
    ...
    _x = torch.arange(0.5, 1.5, 0.01)
    d2l.set_figsize((4.5, 2.5))
    d2l.plot(_x, [f(_x), g(_x)], 'x', 'risk')
...
@app.cell
def _(annotate, d2l, f, torch):
    _x = torch.arange(-1.0, 2.0, 0.01)
    d2l.plot(_x, [f(_x)], 'x', 'f(x)')
...
@app.cell
def _(annotate, d2l, torch):
    _x = torch.arange(-2.0, 2.0, 0.01)
    d2l.plot(_x, [_x ** 3], 'x', 'f(x)')
...
@app.cell
def _(d2l, torch):
    _x, y = torch.meshgrid(torch.linspace(-1.0, 1.0, 101), torch.linspace(-1.0, 1.0, 101))
    z = _x ** 2 - y ** 2
    ax = d2l.plt.figure().add_subplot(111, projection='3d')
    ax.plot_wireframe(_x, y, z, **{'rstride': 10, 'cstride': 10})
    ax.plot([0], [0], [0], 'rx')
    ticks = [-1, 0, 1]
...
@app.cell
def _(annotate, d2l, torch):
    _x = torch.arange(-2.0, 5.0, 0.01)
    d2l.plot(_x, [torch.tanh(_x)], 'x', 'f(x)')
